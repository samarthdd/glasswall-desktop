...
export const docker_exec_rebuild = async (payload: any,request:any,requestId:string
    ...
    LoggerService.addRawLogLine(0,request.filename,'fileName '+request.filename);
    ...
    // Run container ]
    var cmd = 'docker run --rm -v '+'\"'+ configDir+'\"'+ ':/home/glasswall -v '+'\"'+ resolve(inputDir)+'\"'+':/input -v '+ '\"'+resolve(outputDir)+ '\"'+':/output '+
    ...
    exec(cmd, function (err:Error, stdout:string, stderr:string) {      
        ...
        if(err){
            LoggerService.addRawLogLine(0,request.filename,'Error during rebuild -> \n '+err.stack+"\n")
            LoggerService.addLogLine(request.filename,'Error during rebuild -> \n '+err.stack+"\n");
 ...
 //       LoggerService.addRawLogLine(0,request.filename,"Rebuild stdout -> "+String(stdout))             
        LoggerService.addRawLogLine(0,request.filename,"Rebuild stderr -> "+String(stderr))             
        ...
        fs.stat(outputDir+'/Managed', function(err:Error,stat:any) {          
            if (err == null) {                 
                fs.stat(outputDir+'/Managed/'+request.filename, function(err:Error,stat:any) { 
                    if(err == null){
                        LoggerService.addRawLogLine(1,request.filename,'Out file exists');
                        fs.readFile(outputDir+'/Managed/'+request.filename, 'base64', function (err:Error, data:string) {
                            if (err) {
                                LoggerService.addRawLogLine(2,request.filename,'Failed to read output file '+(outputDir+'/Managed/'+request.filename));
                                LoggerService.addLogLine(request.filename,'Failed to read output file '+(outputDir+'/Managed/'+request.filename));
                                resultCallback({'source':sourceFileUrl, 'url':'TBD', 'filename':request.filename, isError:true,
                                    msg:'Failed to read rebuilt file', id:requestId, 
                                    targetDir:folderId, original:request.content,'request':request});
                                    return;
                            }
                            LoggerService.addLogLine(request.filename,'File rebuild successful. Starting analysis');
                            return docker_exec_analysis(payload,request,requestId,folderId,
                                sourceFileUrl,resultCallback,data)               
                          });
                    }
                    else{
                        LoggerService.addRawLogLine(2,request.filename,'File failed rebuuild.Managed dir present. File missing - \n'+err.stack);
                        LoggerService.addLogLine(request.filename,"File failed analysis.Managed dir present.File missing - "+err.stack);
                        resultCallback({'source':sourceFileUrl, 'url':'TBD', 'filename':request.filename, isError:true,
                ...
                LoggerService.addRawLogLine(2,request.filename,'File failed rebuild.Managed dir missing \n'+err.stack);
                LoggerService.addLogLine(request.filename,"File failed rebuild.Managed dir missing - \n"+err.stack);
                resultCallback({'source':sourceFileUrl, 'url':'TBD', 'filename':request.filename, isError:true,
...
export const docker_exec_analysis = async (payload:any,request:any,requestId:string,folderId:string,
    ...
    fs.mkdirSync(inputDir);
    fs.mkdirSync(outputDir);
   // LoggerService.addRawLogLine(0,request.filename,'<docker_exec_analysis> payload '+JSON.stringify(payload));    
    LoggerService.addRawLogLine(1,request.filename,'<docker_exec_analysis> fileName '+request.filename);
    LoggerService.addRawLogLine(1,request.filename,'<docker_exec_analysis> inputDir '+inputDir);
    LoggerService.addRawLogLine(1,request.filename,'<docker_exec_analysis> outputDir '+outputDir);
    var base64Data = payload.Base64.replace(/^data:image\/png;base64,/, "");
    fs.writeFileSync(path.join(inputDir,request.filename),base64Data,{encoding:"base64"});
    LoggerService.addRawLogLine(1,request.filename,"<docker_exec_analysis> Created analysis dirs in "+directory+", inputDir "+inputDir+", outputDir"+outputDir);          
    // Run container        
    let configDir = resolve(Utils.getAppDataPath() + Utils.getPathSep() + 'config');    
    LoggerService.addRawLogLine(1,request.filename,'Config dir - '+(configDir));
    // Run container 
    var cmd = 'docker run --rm -v '+'\"'+ configDir+'\"'+ ':/home/glasswall -v '+
    '\"'+ resolve(inputDir)+'\"'+ ':/input -v '+'\"'+ resolve(outputDir)
    +'\"'+ ':/output '+ RebuildUtils.getRebuildImage() +":" + RebuildUtils.getRebuildImageTag();
    exec(cmd, function (err:Error, stdout:string, stderr:string) {      
        if(err){
            LoggerService.addRawLogLine(2,request.filename,'Error during analysis -> \n '+err.stack+"\n")
            LoggerService.addLogLine(request.filename,'Error during analysis -> \n '+err.stack+"\n");
            resultCallback({'source':sourceFileUrl, 'url':'TBD', 'filename':request.filename, isError:true,
...
export const analyseAnalyzed = async (stdout:string, stderr:string, cmd:string, payload:any,request:any,
 ...
 //       LoggerService.addRawLogLine(0,request.filename,"Analysis stdout -> "+String(stdout))             
        LoggerService.addRawLogLine(1,request.filename,"Analysis stderr -> "+String(stderr))             
        if(stdout.indexOf("error during connect") > -1){
            LoggerService.addLogLine(request.filename,"Docker Daemon is not started");
            resultCallback({'source':sourceFileUrl, 'url':'TBD', 'filename':request.filename, isError:true,
        ...
        fs.stat(outputDir+'/Managed', function(err:Error,stat:any) { 
            if (err == null) {                 
                fs.stat(outputDir+'/Managed/'+request.filename+'.xml', function(err:Error,stat:any) { 
                    if(err == null){
                        LoggerService.addRawLogLine(2,request.filename,'Analysis Out file exists');
                        fs.readFile(outputDir+'/Managed/'+request.filename+'.xml', 'utf8', function (err:Error, data:string) {
                            if (err) {
                                LoggerService.addRawLogLine(2,request.filename,'Failed to read xml file '+(outputDir+'/Managed/'+request.filename)+ '\n '+err.stack );
                                LoggerService.addLogLine(request.filename,'Failed to read xml file '+(outputDir+'/Managed/'+request.filename+'.xml'));
                                resultCallback({'source':sourceFileUrl, 'url':'TBD', 'filename':request.filename, isError:true,
                                    msg:'Failed to read analyzed file', id:requestId, targetDir:folderId, original:request.content});
                                    return;
                            }
                            LoggerService.addLogLine(request.filename,'File analysis successful. Writing results');
                            return writeDecodedBase64File(cleanData, data, request, sourceFileUrl, requestId,
                                folderId, resultCallback);           
                          });
                    }
                    else{
                        LoggerService.addRawLogLine(2,request.filename,'File failed analysis.Managed dir present. File missing - \n'+err.stack);
                        LoggerService.addLogLine(request.filename,"File failed analysis.Managed dir present. File  - \n"+err.stack);
                        resultCallback({'source':sourceFileUrl, 'url':'TBD', 'filename':request.filename, isError:true,
                            msg:'File type not supported', id:requestId, targetDir:folderId, original:request.content,'request':request});
                            return;
                    }
                ...
                LoggerService.addRawLogLine(2,request.filename,'File failed analysis.Managed dir missing \n '+err.stack);
                LoggerService.addLogLine(request.filename,"File failed analysis.Managed dir missing \n"+err.stack);
                resultCallback({'source':sourceFileUrl, 'url':'TBD', 'filename':request.filename, isError:true,
                    msg:'File type not supported', id:requestId, targetDir:folderId, original:request.content});
                    return;
            }
...
export const health_chk = () => {     
    ...
    fs.mkdirSync(inputDir);
    fs.mkdirSync(outputDir);    
    LoggerService.addRawLogLine(1,'-', '<health_chk> License check inputDir '+inputDir);
    LoggerService.addRawLogLine(1,'-', '<health_chk> License check outputDir '+outputDir);
    ...
    LoggerService.addRawLogLine(1,'-', "<health_chk> License check created  dirs in "+directory+", inputDir "+inputDir+", outputDir"+outputDir);
    ...
    else if (!totalOutput.includes(RebuildUtils.getRebuildImage())){
        // Image not present
  ...
  //  LoggerService.addRawLogLine(0,"-", "<health_chk> Image check ouput \n"+totalOutput); 
    totalOutput = "";
    // Run container 
...
//        LoggerService.addRawLogLine(0,"-", "<health_chk> License check response \n "+String(totalOutput))             
        ...
        if (fs.existsSync(path.join(outputDir,'Managed'))) {
            ...
            if(fs.existsSync(outFile)){
                ...
                LoggerService.addRawLogLine(2,"-", "<health_chk> File failed rebuild, Managed dir was there but not the rebuilt file");
                // License not valid
            ...
            LoggerService.addRawLogLine(2,"-", "<health_chk> File failed rebuild");
            localStorage.setItem("healthLogs",oldLogs);       
            return 5;
        }
     }
     else{
        LoggerService.addRawLogLine(2,"-", "<health_chk> Does not have output property");
        localStorage.setItem("healthLogs",oldLogs);       
        return 6;
     }     
     return 7;
...
export const pull_image = () =>{
    ...
    if(pullResponse.hasOwnProperty("output")){            
        ...
        LoggerService.addRawLogLine(1,"-", "<pull_image> Pull Response  = "+totalOutput);        
    }
...
export const check_license = () =>{
    ...
    fs.mkdirSync(inputDir);
    fs.mkdirSync(outputDir);    
    LoggerService.addRawLogLine(1,"-", "check_license  =  inputDir "+inputDir);
    LoggerService.addRawLogLine(1,"-", "check_license  =  "+outputDir);
    var base64Data = Utils.HEALTH_CHK_PNG_BASE64;
    var fileName = Utils.HEALTH_CHK_PNG_NAME;
    fs.writeFileSync(path.join(inputDir,fileName),base64Data,{encoding:"base64"});
    LoggerService.addRawLogLine(1,"-", "check_license  Created rebuild dirs in "+directory+", inputDir "+inputDir+", outputDir"+outputDir);
    var options={"timeout":5000, "shell":false};
    var totalOutput : string;
    totalOutput = "";
    // Run container 
 ...
 //       LoggerService.addRawLogLine(1,"-", "License check output = "+totalOutput);        
        if (fs.existsSync(path.join(outputDir,'Managed'))) {
            const outFile = path.join(outputDir,'Managed',fileName);
            if(fs.existsSync(outFile)){
                oldLogs += "\n"+LoggerService.getLogTime()+" - INFO \n GW LICENSE OK";                
                ...
                LoggerService.addRawLogLine(2,"-", "Licese check file failed rebuild, Managed dir was there but not the rebuilt file");
                // License not valid
            ...
            LoggerService.addRawLogLine(2,"-","license check File failed rebuild");
        ...
        LoggerService.addRawLogLine(2,"-","license check Does not have output property");
